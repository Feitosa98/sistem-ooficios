...
export const GET = withAccess(async (request) => {
  ...
  const search = term ? or(...[letters.subject, letters.recipient, letters.reference, letters.department, letters.suffix].map((field) => sql`lower(${field}) LIKE ${match} ESCAPE '!' `), sql`concat(lpad(${letters.number}, 3, '0'), '/', ${letters.year}) LIKE ${match} ESCAPE '!' `) : undefined;
  const filter = and(search, status !== "Todos" ? eq(letters.status, status) : undefined);
  const db = getDb();
  const [rows, totals, grouped, maximum, recent] = await Promise.all([
...
export const POST = withAccess(async (request) => {
  ...
  try {
    const db = getDb();
    const [inserted] = await db.insert(letters).values(input.data).$returningId();
    const [letter] = await db.select().from(letters).where(eq(letters.id, inserted.id)).limit(1);
