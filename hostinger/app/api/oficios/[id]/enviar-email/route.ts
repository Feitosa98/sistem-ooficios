...
import { isUniqueViolation } from "@/lib/letter-validation";
...
export const POST = withAccess(async (request, context: RouteContext, actor) => {
  ...
  let claimed = true;
  try { await db.insert(emailDeliveries).values({ id: deliveryId, letterId: id, ownerEmail: actor.email, recipient: input.data.recipientEmail, documentKey: letter.signedFileKey, requestHash, state: "sending" }); }
  catch(error) { if(!isUniqueViolation(error))throw error; claimed=false; }
  if (!claimed) {
  ...
  try {
    ...
    const current = await db.transaction(async (tx) => {
      await tx.update(emailDeliveries).set({state:"sent",providerId:result.providerId||null,updatedAt:sql`CURRENT_TIMESTAMP`}).where(eq(emailDeliveries.id,deliveryId));
      await tx.update(letters).set({status:"Enviado",sentAt:sql`CURRENT_TIMESTAMP`,recipientEmail:input.data.recipientEmail,version:sql`${letters.version}+1`,updatedAt:sql`CURRENT_TIMESTAMP`}).where(and(eq(letters.id,id),eq(letters.signedFileKey,letter.signedFileKey!),inArray(letters.status,["Assinado","Enviado"])));
      return (await tx.select().from(letters).where(eq(letters.id,id)))[0];
    });
