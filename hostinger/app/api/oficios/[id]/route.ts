...
export const PATCH = withAccess(async (request, context: RouteContext) => {
  ...
  if (statusOnly) {
    ...
    changes = { status, ...(status === "Enviado" && payload.sentByEmail === true ? { sentAt: new Date().toISOString().slice(0,19).replace("T"," "), recipientEmail } : {}) };
  } else {
  ...
  const [result] = await db.update(letters).set({ ...changes, version: sql`${letters.version} + 1`, updatedAt: sql`CURRENT_TIMESTAMP` }).where(and(eq(letters.id, id), eq(letters.version, current.version), ...(!statusOnly ? [isNull(letters.signedFileKey)] : [])));
  if (!result.affectedRows) throw new HttpError(409, "Outra operação alterou este ofício. Atualize a lista.");
