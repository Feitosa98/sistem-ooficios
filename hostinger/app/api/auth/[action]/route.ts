...
const schemas={
 ...
 password:z.object({token:z.string().max(100),password:z.string().min(12).max(128)}),
...
export async function POST(request:Request,context:{params:Promise<{action:string}>}){
 ...
 try{
  ...
  if(action==="logout"){await logout();response=Response.json({success:true});}
   ...
   if(action==="login"){const input=schemas.login.parse(payload);response=Response.json({challenge:await login(input.email,input.password)});}
   else if(action==="setup"){await requestSetup(schemas.setup.parse(payload).email);response=Response.json({message:"Se o e-mail estiver autorizado, você receberá um link para confirmar o endereço e definir sua senha."});}
