...
export function renderEmailHtml(options: EmailTemplateOptions & { code?: string }): string {
  ...
  if (codeMatch && isCodeEmail) {
    contentHtml = `
      <p style="margin:0 0 16px;font-size:15px;line-height:24px;color:#213b36;">
      ...
      <table role="presentation" cellspacing="0" cellpadding="0" align="center" style="margin:26px auto 28px;background:#fdfbf7;border:2px dashed #a68845;border-radius:12px;box-shadow:0 4px 14px rgba(166,136,69,0.12);">
        <tr>
          <td style="padding:22px 36px;text-align:center;">
            ...
            <div style="font-family:'Courier New',Courier,Consolas,monospace;font-size:42px;font-weight:800;letter-spacing:14px;color:#183b32;padding-left:14px;line-height:46px;">
              ${escapeHtml(codeMatch)}
            </div>
          </td>
        </tr>
      </table>
      <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="background:#f4f6f4;border-left:4px solid #2d695c;border-radius:0 8px 8px 0;margin:0 0 20px;text-align:left;">
        <tr>
          <td style="padding:14px 18px;font-size:13px;line-height:21px;color:#354e47;">
    ...
    contentHtml = options.body.replace(/\r\n?/g, "\n").split(/\n\s*\n/).map((paragraph) => {
      ...
      const content = /^https?:\/\/[^\s<>"']+$/i.test(text)
        ? `<div style="text-align:center;margin:24px 0;"><a href="${escapeHtml(text)}" target="_blank" style="background:#183b32;border:1px solid #a68845;border-radius:8px;color:#ffffff;display:inline-block;font-size:15px;font-weight:700;line-height:18px;padding:15px 32px;text-decoration:none;box-shadow:0 4px 12px rgba(24,59,50,0.15);">Confirmar no Sistema &rarr;</a><br><span style="display:block;margin-top:12px;font-size:11px;color:#66736f;word-break:break-all;">Link direto: <a href="${escapeHtml(text)}" style="color:#286454;text-decoration:underline;">${escapeHtml(text)}</a></span></div>`
...
<tr><td style="padding:26px 24px;background:#183b32;border-bottom:4px solid #a68845;border-radius:12px 12px 0 0;word-break:break-word;overflow-wrap:anywhere;text-align:center;"><p style="margin:0 0 6px;font-size:11px;line-height:16px;letter-spacing:2px;color:#e2cf9f;text-transform:uppercase;font-weight:700;">Cartório 2º Ofício de Manacapuru/AM</p><p style="margin:0;font-size:22px;line-height:29px;font-weight:bold;color:#ffffff;font-family:Georgia,serif;">${escapeHtml(options.fromName)}</p></td></tr>
<tr><td style="padding:28px 24px 8px;word-break:break-word;overflow-wrap:anywhere;"><h1 style="margin:0 0 22px;font-size:22px;line-height:30px;color:#183b32;font-family:Georgia,serif;">${escapeHtml(options.subject)}</h1><div style="font-size:15px;line-height:26px;">${contentHtml}</div></td></tr>
${attachment}
