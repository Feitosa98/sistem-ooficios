...
export async function savePkiConfig(config: { licenseKey?: string; restPkiToken?: string; clearRestPkiToken?: boolean }) {
  ...
  await getDb().insert(systemSettings).values({ key: "pki_settings", value: JSON.stringify(updated) }).onDuplicateKeyUpdate({ set: { value: JSON.stringify(updated), updatedAt: sql`CURRENT_TIMESTAMP` } });
