...
export function AuthForm({ setup=false }: { setup?:boolean }) {
 ...
 async function submit(event:FormEvent){
  ...
  try{
   ...
   if(mode==="login"){
     if(result.directLogin){window.location.replace("/");}
     else{setChallenge(result.challenge);setPassword("");setMode("mfa");}
   }
 ...
 if(mode==="done")return <div className="auth-fields"><p role="status">E-mail confirmado e senha salva. Entre para acessar o sistema.</p><Button asChild className="login-submit"><a href="/">Ir para o login</a></Button></div>;
 return <form onSubmit={submit} className="auth-fields">
  {mode==="mfa"?<><p className="auth-note">Enviamos um código para <strong>{email}</strong>. Ele expira em dez minutos.</p><label htmlFor="auth-code">Código de confirmação</label><Input id="auth-code" required autoFocus inputMode="numeric" autoComplete="one-time-code" pattern="[0-9]{6}" maxLength={6} value={code} onChange={e=>setCode(e.target.value.replace(/\D/g,""))} placeholder="000000" className="auth-code" /></>
   :mode==="password"?<><p className="auth-note">Defina uma senha com pelo menos 8 caracteres.</p><label htmlFor="auth-new-password">Nova senha</label><Input id="auth-new-password" required type="password" minLength={8} maxLength={128} autoComplete="new-password" value={password} onChange={e=>setPassword(e.target.value)} /><label htmlFor="auth-confirm">Confirmar senha</label><Input id="auth-confirm" required type="password" minLength={8} maxLength={128} autoComplete="new-password" value={confirm} onChange={e=>setConfirm(e.target.value)} /></>
   :<><label htmlFor="auth-email">E-mail</label><Input id="auth-email" type="email" required autoComplete="username" maxLength={254} value={email} onChange={e=>setEmail(e.target.value)} placeholder="seu@email.com" />{mode==="login"&&<><label htmlFor="auth-password">Senha</label><Input id="auth-password" required type="password" minLength={8} maxLength={128} autoComplete="current-password" value={password} onChange={e=>setPassword(e.target.value)} placeholder="Mínimo de 8 caracteres" /><p className="auth-note" style={{fontSize:"0.78rem",marginTop:"0.25rem",lineHeight:"1.3"}}>Primeiro acesso do administrador: informe o e-mail cadastrado e sua nova senha para ativar o acesso.</p></>}</>}
