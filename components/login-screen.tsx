...
import { ArrowRight, FileText, LockKeyhole, ShieldCheck, CircleHelp } from "lucide-react";
import { Button } from "@/components/ui/button";
import { chatGPTSignInPath, chatGPTSignOutPath } from "@/app/chatgpt-auth";

export function LoginScreen({ status = 401 }: { status?: number }) {
  const pending = status === 403;
  const unavailable = status !== 401 && !pending;
  const title = pending ? "Acesso pendente" : unavailable ? "Voltamos em breve" : "Bem-vindo ao seu espaço de trabalho.";
      ...
      <section className="login-entry" aria-labelledby="login-title">
        <div className="login-card">
          ...
          <Button asChild className="login-submit">
            <a href={pending ? chatGPTSignOutPath() : unavailable ? "/" : chatGPTSignInPath("/")}>
              {pending ? "Entrar com outra conta" : unavailable ? "Tentar novamente" : "Entrar com ChatGPT"}
              <ArrowRight size={18} aria-hidden="true" />
            </a>
          </Button>
          {!pending && !unavailable && <p className="login-provider-note">Use a conta associada ao e-mail cadastrado pelo administrador.</p>}
          ...
          <details className="login-help">
            ...
            <p>Solicite ao administrador o cadastro do seu e-mail e a liberação do acesso. Se já tiver acesso, confira se está usando a conta correta.</p>
          </details>
        </div>
